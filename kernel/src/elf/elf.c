...
unsigned mmap_anonymous(unsigned length)
{
    ...
    memset(pa_to_va(physaddr), 0, length);
    return virtaddr;
}
static unsigned alloc_one_clean_page(unsigned virtaddr)
{
    ...
    //printk("%08X ===> %08X\n", virtaddr, physaddr);
    memset(pa_to_va(physaddr), 0, PAGE_SIZE);
    return physaddr;
}
...
static void load_segment_from_elf(unsigned destmem, size_t memsz, size_t elfoff, size_t filesz)
{
    ...
    int pgoffset = destmem & 0xfff;
    memsz += pgoffset;
    destmem = destmem & 0xfffff000;
    //int cur_page = 0;
    ...
    while (filesz > 0) {
        ...
        dest = pa_to_va(alloc_one_clean_page(destmem)); // new page should zero-filled
